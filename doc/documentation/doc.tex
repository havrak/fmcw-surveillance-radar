...
\begin{figure}[h!]
	...
	\caption[\sidar, Downloaded from: \url{https://wiki.ffo.indiesemi.com/wiki/images/2/29/Figure_2_24_GHz_configuration.png} Visited on \text{[2024-12-28]}]{\sidar  24~GHz configuration}
	\label{fig:sidar}
\end{figure}
...
It's designed to broadcast primarily in the ISM band (24.0-24.25~GHz) additionally ultra-wide bandwidth mode is also supported (23-26~GHz) however this mode cannot be enabled on the development kit.
In its main band transmitter output power is in the range of 2.5-6~dBm depending on the configuration \cite{sidarTRX}.
Maximal range of

As showed on picture \ref{fig:sidar} the chip is connected to two microstrip patch antennas.
Patches are placed in relatively standard configuration forming a 6 by 4 array with spacing roughly equal to half of the wavelength.
...
Dimensions of the array were taken using a classical optical microscope and whole array was redrawn in CST studio.
Substrate parameters also weren't provided by the manufacturer however in TRX-024-007 datasheet there is a board stack up for the evalutions board of the chip itself.


\section{122~GHz Header}

122~GHz header is based on TRX-120-001 transceiver that aside from basic components needed to transmit and recive RF also include two on-chip antennas.
