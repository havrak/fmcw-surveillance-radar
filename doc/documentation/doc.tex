...
\boldred{Would like to expand in this section a little more, but IEEE SSO login is down and I can't access the papers.}
...
\begin{figure}[h!]
	...
	\caption[\sidar, Downloaded from: \url{https://wiki.ffo.indiesemi.com/wiki/images/2/29/Figure_2_24_GHz_configuration.png} on \[2024-12-28\]]{\sidar  24~GHz configuration}
	\label{fig:sidar}
\end{figure}

Communication with the radar board itself isn't possible, or at least  the communication interface is not documented.
In order to configure the radar or read its measurements user is either connected to on board ESP chip wirelessly or to STM32 Nucleo series microcontroller over UART (facilitated with mini USB 2.0 connection).
Both are relatively low bandwidth communication -- the serial maxing at baudrate of 1~000~0000.

Low level communication with the radar system is not possible and by default radar doesn't even forward raw data from ADC to the user.
Instead a on board DSP takes place with 1D FFT being calculated and targets classified.
