...
Were we to use 24~GHz header with 25~ms trigger we get maximal speed of 0.124~m/s and even with low 8 NFFT steps the resolution is only $1.9\cdot 10^{-5}$~ms.
...
Second cube is 3D structure with dimensions of fast time $\times$ slow time $\times$ range, this one is used to store CFAR output.
...
\begin{enumerate}
  \item Data are read, simple FFT is calculated and data are buffered into a circular buffer.
  \item Data re retrived in circular buffer paird with positions are processed -- CFAR or speed calculation. This step runs in a separate thread.
  \item Processed data are buffered into a ping-pong buffer. In case buffer is full the cube update routine is launched.
  \item After cubes are updated data re visualized in user chosen format.
\end{enumerate}

\subsection{1st step -- Data acquisition}
...
%   decision/.style={diamond, aspect=1.5, minimum width=2cm, minimum height=0.8cm, text centered, draw=black, fill=green!30},
%   arrow/.style={thick,->,>=stealth}
% ]
%


\subsection{2nd step -- 2D FFT and CFAR calculation}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=1cm, scale=0.7]
    ...
    \node (direct_update) [process, below=of use_spread, align=center] {Update\\ cube};
    \node (create_subcube) [process, below=of direct_update] {Create subcube};
		\node (aggregate) [process, below =of create_subcube, align=center] {Aggregate updates\\Update cube};
    \node (keep_cfar) [decision, below=2cm of keep_raw] {Keep CFAR?};
    \node (update_cfar) [process, below=of keep_cfar, align=center] {Update CFAR\\ cube};
    ...
    \draw [arrow] (create_subcube) -- (aggregate);
		\draw [arrow] (aggregate.west) -| ++(-0.5,5.922 ) -| (keep_cfar.north);
    % \draw [arrow] (aggregate.south) |- (keep_cfar.east);
    % \draw [arrow] (keep_raw.east) -| ++(1.5,0) |- (keep_cfar.north);
    %
