...
Here output from radar is displayed, on the sidebar are buttons to establish serial connections.
Lastly from the topbar user can access preferences and platform control interface.
...
Firstly most limits GUI imposes are guided more by mathematical requirements and what configuration allows than by practical limitations.
...
Figure \ref{fig:preferences} shows the preferences menu, user can see that there are four main sections.
First configures serial port connections to the radar and the platform.
Second handles configuration of the platform, followed by a section designated for radar configuration.
...
User's preferences are stored in a INI file format in directory whose path depends on the operating system -- \texttt{\%APPDATA\%/Local/fmcw/fmcw.conf} on Windows and \texttt{\$HOME/.config/fmcw/fmcw.conf} for unix based systems.
...
Lastly user can toggle wheter platform debug message should be displayed.
...
Program persists in the same configuration file as preferences.

Left sidebar allows user to load stored programs by clicking on their name.
Middle text boxes are used to edit programs header (top part) and program body (bottom part).
...
Radar data processing part can then request list of positions for given time interval.

Platform control can also emit an event when platform finds itself close to a predetermined position, specificed in preferences (Figure \ref{fig:preferences}) option \texttt{Yaw reset}.
This event is used to request zeroing of the radar cube in case \texttt{Old removal} method is switched to \texttt{Yaw} option.
...
Radar frequency toggles between 24~GHz and 122~GHz header, this need to be done in conjunction with reflashing of the radar's firmware.
...
While manufacturer statesd 3~GHz limit on the 24~GHz header and 5~GHz on the 122~GHz header \cite{sidarPRO}, in practice, especially with the 24~GHz header, achievable bandwidth was usually much lower.

User can also configure the number of samples per chirp and ADC sampling rate -- based on these two paramters influence time of a single ramp which is calcualted with equation \ref{eq:sampling}.
...
Last paramters is trigger period which dictates how often will computer send a trigger to the radar to start the next chirp.
...
After retrieval basic 1D FFT is run on windowed data (Hamming window is used.) and the result is again buffered.
This steps leaves us with mutliple timestamp FFT spectrums -- allowing us to calculate range-doppler map.

Program then verifies if the platform has moved (Unless user disabled this check.).
This check enables to save processing resources by not computing R-D map that would be immediately rewritten.
Afte the check chirps and their platform position data are retrieved and further processing is launched in a parallel thread.
...
After RCS conversion based on user preferences CFAR can be calculated.
CFAR is a simple algorithm that enables basic target detection in the radar data.
This is done by comparing value at given range bin againts average values accross it's neighborhood, that separated by bin under test by a number of guard bins \cite{richards2022}.
...
If raw data calculation is enabled the program takes one of two paths depending on whether speed calculation is toggled on or off.
...
Both $\varepsilon$ and $N$ parameters are left to the user to configured and are constant.
In addition scaling for range can be introduced if needed to achieve better results.
