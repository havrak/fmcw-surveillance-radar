...
However doing some simple subtraction in spectrogram of the both signals isn't really feasible -- the calculation needs to take a smarter approach.


Let us define the chirp slope $k_{sw}$ we can describe the change in frequency of the received signal as
\begin{equation}
	\Delta f_s(t) = k_{sw}t = \frac{BW}{T_{ch}} t,
	\label{eq:deltaf}
\end{equation}
where $t$ is the time goes from 0 to chirp length $T_{ch}$. Standard equation of FM signal can be written as
\begin{equation}
	s(t) = A \cos\left(\omega_c t + \int_{0}^{t} f(s) dt\right),
	\label{eq:fm}
\end{equation}
