...
Unfortunately radar doesn't maintain constant intervals between chirps which severely limits accuracy of speed measurement.
...
Concrete meaning of different settings will be outlined in the following sections.
...
Second handles configuration of the platform followed by section for radar settings, which is sent to the radar.
...
User's preferences are stored in a INI file format in directory depending on the operating system.
...
Managing platform programs is done with a separate window shown in figure \ref{fig:platformControl}.
...
Given that radar output is displayed in the main window there isn't any specialized interface for communication with the radar.
Still in preferences menu (figure \ref{fig:preferences}) there are some paramters that can be configured.
Radar frequency toggles between 24~GHz and 122~GHz header, though it is neccesary to not that in order to change the header the radar must have it's firmware reflashed with corresponding version.
...
Another paramters that are sent to the radar are radar's bandwidth and amplifier gain settings.

Last paramters is trigger period which dictates how often will computer send trigger to the radar to start the next chirp.
As previously stated the radar is at best capable of around 50~Hz update rate or 20ms per chirp.
...
Any more complex visualization styles would be too cluttered and hard to interpret.
...
Lower bound is limited by the radar's mounting bracket and upper bound is more or less arbitrary.
...
If data can't be read in time attributed timestamps will be incorrect and visualization will be wrong.
...
We can split processing into multiple distinct steps.
\begin{enumerate}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=1.5cm, scale=0.85]
    ...
    \node (processor_header) [layerheader] at (6, 0) {dataProcessor.m};
  ...
  \end{tikzpicture}
  \caption{Processing flow - 1st step}
...
Based on user preferences CFAR can be calculated, which enables basic target detection in the radar data \cite{richards2022}.
As simple 1D version is used matlab's built in implementation as part of Phased Array System Toolbox is used \cite{matlab_cfar}.
Given that cell can vary widely in width depending on chosen bandwidth both number of guard cells and the number of training cells are left to be configured by the user.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=1cm, scale=0.7]
    ...
    \node (raw_decision) [decision, right=1.3cm of cfar_decision] {RAW?};
    \node (speed_decision) [decision, right=1.8cm of raw_decision] {Speed?};
    ...
    % Speed branch
    \node (mock) [process, below left=1.5cm and -0.4cm of speed_decision, align=center] {Mock R-G\\ map};
  ...
  \end{tikzpicture}
  \caption{Processing flow - 2nd step (Feature Processing)}
  \label{fig:feature_flow}
\end{figure}

\section{Cube update}

Last step of processing handles updating the radar cube structure.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=1cm, scale=0.7]
    ...
    \node (cube_header) [layerheader] at (8, 0) {radarDataCube.m};
    ...
    \draw [arrow] (buffer_full.west) -- ++(-1,0) node[left] {No} |- (end_func.west);
    %
    ...
    \draw [arrow] (aggregate2.west) -| ++(-0.5,8.856 ) -| (keep_cfar.north);
    %
    \draw [arrow] (keep_cfar.west) -- ++(-1,0) node[left] {No} |- (cube_return.west);
    \draw [arrow] (keep_cfar.south) -- node[right] {Yes} (update_cfar.north);
    \draw [arrow] (update_cfar.south) -- (cube_return.north);
    %
  \end{tikzpicture}
  \caption{Processing flow - Final stage (Data Cube Management)}
  \label{fig:cube_flow}
\end{figure}

Update routine for CFAR cube and raw cube, in case spread pattern is disabled, are quite similar.
...
However the application offers a number of different visualization styles.

\begin{enumerate}
	\item Range-Azimuth map: Data are displayed in a polar plot with variable range and azimuth while pitch is fixed.
	\item Range-Doppler map / Range-RCS map: Range doppler map for a single fixed pitch and yaw angle is displayed. In case speed calculation is disabled the data showed as a simple range-RCS plot. Only raw data are displayed in this method.
	\item Target-3D: CFAR data are displayed in 3D cartesian space. In case DBSCAN is turned on clustering is run on CFARed date before.
\end{enumerate}

\subsection{Range-Azimuth map}

As opposed to calculating projection into horizontal plane data are displayed ina  conical projection - that is no calculations are run on the angle and simply the cone we get by fixing the pitch angle is stretched to a circle.
The data visualized changed based on user preferences either only raw data are displayed or CFAR or sum of both.
In case of raw data it is not possible to display both targets RCS value and speed thus the data the cube is first summed up in the speed dimension.

To control which angle is displayed user can use the textbox at the bottom left corner of the screen.
Update of the plot takes place right after next cube update is finished,
