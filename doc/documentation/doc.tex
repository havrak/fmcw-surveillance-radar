...
This lack of perforace in this area severely limits the radar's usability with regard to surveillance.
Most surveillance applications are designed to monitor dynamic environments, where targets is moving.
And usually at much higher speeds than the \sidar can measure.
This radar is completely unsuitable for such applications and is primarily useful for static environments.
...
This event allows reset of stored data after one cycle is completed.
...
In case cube is set to decay (\texttt{Old removal} option) whole cube is multiplied by a decay factor that is derived from platforms movement speed.
...
Both $\varepsilon$ and $N$ parameters are left to the user to configure with \texttt{DBSCAN epsilon} and \texttt{DBSCAN min detections}.
