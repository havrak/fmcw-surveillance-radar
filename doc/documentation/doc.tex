...
A maximal range of 400 m is advertised \cite{sidarMANOld}, though in practice only ranges of around 250~m were achieved, with target being a multistory building.
...
This location is specified in the \texttt{Reset yaw} option, with a tolerance zone fixed at two degrees on either side.
In order to prevent multiple triggering of the event, the code will only emit it once per 1~s.
This event allows for a reset of stored data after a cycle is completed.
That is when the \texttt{Cube decay} option is set to reset, if not no reaction takes place.
