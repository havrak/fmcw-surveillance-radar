...
In case cube is set to decay whole cube is multipled by decay factor that is derived from speed (The slower the platform moves the slower the decay).
...
Instead of adding new data just to one position in the cube the range doppler maps will be spread over a larger area with a aid of additional 2D matrix.
This matrix contains a weighting pattern made up on 2D Gaussian function.
As subsequent data are usually for similar position instead of constantly updating the whole cube smaller subcube is created into which these contributions are added.
Than the whole subcube is added to the original cube.
