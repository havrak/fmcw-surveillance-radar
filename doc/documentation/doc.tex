...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[scale=0.9, node distance=1.5cm]
    ...
    \draw [arrow] (send_response.west) -- ++(-0.8, 0) |- (wait_serial.west);
...
If the user wishes for processing to always take place (e.g., when verifying radar settings while the platform is static) this check can be skipped by toggling \texttt{Require pos change}.
